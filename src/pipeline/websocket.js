...
class WebSocket extends EventEmitter {
  ...
  _connect () {
    const connectErrorListener = (err) => {
      ...
      switch (err.code) {
        case 'ECONNREFUSED':
          Logger.info(`Error connecting to pipeline, attempting to connect after sleeping ${this._reconnectInterval}ms`)
  ...
  _connectTimedOut () {
    this._cleanup()
    this.emit('error', new Error(`Unable to connect to pipeline within ${this._connectTimeout}ms`))
  }
